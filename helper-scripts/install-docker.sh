...
add_line_to_file_if_not_exist 'Acquire::http::Proxy::download.docker.com "http://freepkg.dev:8118/";' /etc/apt/apt.conf.d/proxy.conf
...
curl -x http://freepkg.dev:8118 https://get.docker.com -o get-docker.sh
# Create a curlrc file for proxying and install docker
add_line_to_file_if_not_exist 'proxy="http://freepkg.dev:8118/"' .curlrc
